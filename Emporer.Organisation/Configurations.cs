...
namespace OpenResKit.Organisation
{
  ...
  public class EmployeeConfiguration: EntityTypeConfiguration<Employee>, IDomainEntityConfiguration
  {
    ...
    {
      //HasMany(s => s.Groups).WithMany();
      //HasMany(s => s.Groups).WithOptional().HasForeignKey(l => l.Id);
      HasMany(s => s.Groups).WithMany();
    }
    public void AddConfigurationToModel(ConfigurationRegistrar configurations)
    {
  ...
  public class SeriesConfiguration: EntityTypeConfiguration<Series>, IDomainEntityConfiguration
  {
    ...
    {
      Ignore(x => x.CycleEnum);
      HasMany(s => s.WeekDays).WithOptional().WillCascadeOnDelete();
  ...
  public class ScheduledTaskConfiguration: EntityTypeConfiguration<ScheduledTask>, IDomainEntityConfiguration
  {
    ...
    {
      //HasOptional(st => st.DueDate).WithOptionalPrincipal().WillCascadeOnDelete();
	    HasRequired(st => st.DueDate).WithOptional().WillCascadeOnDelete(true);
			//HasOptional(st => st.DueDate).WithOptionalPrincipal();
	    //HasOptional(st => st.EntryDate).WithOptionalPrincipal().WillCascadeOnDelete();
    }
    public void AddConfigurationToModel(ConfigurationRegistrar configurations)
    {
      configurations.Add(this);
